...
if(isset($_POST['place_order'])){
   ...
   $verify_cart = $conn->prepare("SELECT * FROM `rfq_item_list` WHERE user_id = ?");
   $verify_cart->execute([$user_id]);
   
   if(isset($_GET['get_id'])){

      $get_product = $conn->prepare("SELECT * FROM `products_entity` WHERE entity_id = ? LIMIT 1");
      ...
      if($insert_order){
         $delete_cart_id = $conn->prepare("DELETE FROM `rfq_item_list` WHERE user_id = ?");
   ...
   <div class="row">
      ...
      <div class="summary">
         ...
         <?php
            $grand_total = 0;
            if(isset($_GET['get_id'])){
               $select_get = $conn->prepare("SELECT * FROM `products_entity` WHERE entity_id = ?");
         ...
         <?php
               ...
               $select_cart = $conn->prepare("SELECT * FROM `rfq_item_list` WHERE user_id = ?");
               $select_cart->execute([$user_id]);
               if($select_cart->rowCount() > 0){
                  while($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)){
                     $select_products = $conn->prepare("SELECT * FROM `products_entity` WHERE entity_id = ?");
